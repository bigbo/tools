 ...
 */
require_once( "$IP/extensions/PassportAuth/passport.inc.php" );
//hook for 
$wgExtensionFunctions[] = 'Auth_remote_user_hook';
function Auth_remote_user_hook(){
    global $_SESSION, $wgUser;
    $username = $_SESSION["passport_username"];
    $token = isset($_SESSION["wsLoginToken"]) ? $_SESSION["wsLoginToken"] : ""; 
    $trycount = 0;
    $errormessage = '';
    do {
            $tryagain = false;
...
class PassportAuthPlugin extends AuthPlugin{
	/**
	 * Check whether there exists a user account with the given name.
	 * The name will be normalized to MediaWiki's requirements, so
	 * you might need to munge it (for instance, for lowercase initial * letters).  * * @param $username String: username.
	 * @return bool
	 */
	...
	public function authenticate( $username, $password ) {
                global $passport_username;
                if(strtolower($username) == $passport_username){
			return true;
                }
		return false;
	}
	 ...
	 * Return true if external accounts can be created.  * @return Boolean */
	...
	public function getUserInstance( User &$user ) {
		return new PassportAuthPluginUser( $user );
	}
